function [codes,clusters,err,init_centroids] = som_kmeans(method,D,k,epochs,verbose)
...
init_centroids = centroids; % dan addition
clusters       = zeros(1,l);
...
switch method
  ...
  case 'batch',
      ...
      while iter<epochs
        ...
        [~,clusters] = min(((ones(k, 1) * sum((data.^2)', 1))' + ...
                   ones(l, 1) * sum((centroids.^2)',1) - ...
                   2.*(data*(centroids')))'); % each point gets the closest center - 'min' dist

        for i = 1:k
          f = find(clusters==i);
          s = length(f);
          if s, centroids(i,:) = sum(data(f,:)) / s; end % update new centers (centroids)
        end

        if iter 
          if sum(old_clusters==clusters)==0 % another stopping condition if centroids are not changed
            if verbose
                fprintf(1, 'Convergence in %d iterations\n', iter);
            end
            break; 
          end
        end
      ...
      [~,clusters] = min(((ones(k, 1) * sum((data.^2)', 1))' + ...
                  ones(l, 1) * sum((centroids.^2)',1) - ...
                  2.*(data*(centroids')))'); % update new colsest center to each data point
  otherwise,
