function [final_2TBN] = combine_local_struct_forWeights(pdags,num_obs,t_star)
...
% [t_star]  - (scalar) index of the reference current local graph
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
...
nbrs=cell(1,length(pdags));
%init neighbour counter matrix. S in the paper
for i=1:length(pdags)
    if ~isempty(pdags{i})
        nbrs{i}=zeros(num_obs,num_obs);
        for j=1:num_obs
            ...
            for k=1:length(cld)
                if(cld(k)<=num_obs && cld(k)~=j) %the child is not a latent
                    nbrs{i}(j,cld(k))=nbrs{i}(j,cld(k))+1;
                end
            end
        end
    end
...
%each local graph is affected from all nbrg graphs (weighted)
nbrs_new=zeros(num_obs,num_obs);
for t=1:length(pdags)
    if ~isempty(nbrs{t})
        weight=calculate_weights(t,t_star,length(nbrs));
        for i=1:num_obs
            for j=1:num_obs
                nbrs_new(i,j)=nbrs_new(i,j)+nbrs{t}(i,j)*weight;
            end
        end
    end
end
nbrs_new(nbrs_new>=0.5)=1; %conf=0.5
nbrs_new(nbrs_new<0.5)=0;
nbrs=nbrs_new;
...
%%
function [w] = calculate_weights(t,t_star,T)
%calculate weight baset on time-distance

h=3;%kernel bandwidth

kh=exp(-((t-t_star)^2/h));%Gaussian RBF kernel
denominator=0;
for i=1:T
    denominator=denominator+exp(-((i-t_star)^2/h));
end
w=kh/denominator;
%%