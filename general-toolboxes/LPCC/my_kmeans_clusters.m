function [centers,init_centers,clusters,errors,ind] = my_kmeans_clusters(sD,n_max,n_min,c_max,verbose)
...
%   [n_min]   (scalar) minimum number of clusters, default is 2
...
if nargin < 3 || isempty(n_min)   || isnan(n_min), n_min = 2; end
if nargin < 4 || isempty(c_max)   || isnan(c_max), c_max = 5; end
if nargin < 5 || isempty(verbose) || isnan(verbose), verbose = 0; end

centers      = cell(n_max,1);
init_centers = cell(n_max,1); % dan addition
...
centers{1}      = m;
init_centers{1} = m; % dan addition
clusters{1}     = ones(dlen,1);
[~,qerr]        = som_bmus(m,D);
errors(1)       = sum(qerr.^2);
ind(1)          = NaN;

if verbose, fprintf(2,'Doing k-means for %d-%d clusters\n',n_min,n_max); end

for i = n_min:n_max, % number of clusters
    ...
    for j = 1:c_max % run number j for cluster i      
        ...
        [c,lbl,err,ic] = som_kmeans('batch', D, i, 100, 0); %ic - dan added. stands for init centers
        if err < best, lbl_best = lbl'; c_best = c; best = err; init_c_best=ic; end % 'added in k_best = k'; by kr 1.10.02
    end
    if verbose, fprintf(1, '\n');  end

    % store the results (best random run for each k)  
    centers{i}      = c_best;
    init_centers{i} = init_c_best; % dan addition
    clusters{i}     = lbl_best;
    errors(i)       = best;
    ind(i)          = db_index(D, lbl_best, c_best, 2); % modified by kr 1.10.02
